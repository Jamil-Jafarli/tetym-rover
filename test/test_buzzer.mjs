...
console.log('\nBu makinede pin var mı — dürüstçe');
{
  ...
  if (process.platform === 'linux') {
    ok(['sysfs', 'pinctrl', 'none'].includes(st.backend), `Linux: backend ${st.backend}`);
  } else {
    ok(st.backend === 'none' && done === false, 'Pi olmayan makinede pin sürülmüyor');
    ok(/Raspberry Pi değil/.test(st.error || ''), `ve sebebi yazılıyor  (${st.error})`);
  }
...
console.log('\nSunucuda: ayar kaydediliyor, geri vitese göre ötüyor');
{
  ...
  const proc = spawn('node', ['server.js', '--marlin', '--no-connect', '--http', '18203',
  ...
  try {
    ...
    await sleep(200);
    ok(/pin refused: GPIO14/.test(out), 'listede olmayan pin sürülmüyor (seri konsol pini)');
    ws.send(JSON.stringify({ cmd: 'pi_pin', pin: 17, value: 1 }));
    await sleep(250);
    ok(/GPIO17 = 1/.test(out), 'listedeki çıkış pini sürülüyor');
    ws.close();
