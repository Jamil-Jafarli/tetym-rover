/**
 * The page, in a real browser, with a fake printer behind it.
 *
 * The unit tests cover the G-code the server puts on the wire. What they
 * cannot see is a page that throws on load — and a page that throws on load
 * looks exactly like a page that is fine until you hold a key and nothing
 * moves. So: open it, fail on any console error, then drive it with the
 * keyboard and assert on the requests that actually left the browser.
 *
 * /api/marlin is intercepted rather than served, so this runs with no printer
 * and no serial port. What it is testing is the half that lives in the page:
 * key -> vector -> request body.
 */
...
const server = spawn('node',
  ['server.js', '--http', '8199', '--host', '127.0.0.1', '--no-connect'],
